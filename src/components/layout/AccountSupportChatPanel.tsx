...
const AccountSupportChatPanel = ({
  ...
  const [selectedProducts, setSelectedProducts] = useState<
    ChatComposerProduct[]
  >([]);
  const [selectedOrders, setSelectedOrders] = useState<ChatOrderAttachment[]>(
    [],
  );
  ...
  useEffect(() => {
    return () => {
      imageItemsRef.current.forEach(({ previewUrl }) =>
  ...
  useEffect(() => {
    if (!prefillProduct && !prefillOrder) {
      return;
    }

    if (prefillProduct) {
      setSelectedProducts((prev) => {
        if (prev.some((item) => item.id === prefillProduct.id)) return prev;
    ...
    if (prefillOrder) {
      setSelectedOrders((prev) => {
        if (prev.some((item) => item.id === prefillOrder.id)) return prev;
        return [...prev, prefillOrder];
      });
    }

    onPrefillConsumed?.();
  }, [prefillProduct, prefillOrder, onPrefillConsumed]);
    ...
    return (
      <div className="mt-2 space-y-2">
        {attachments.map((attachment) => {
                ...
                <div className="flex gap-3 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      ...
                      {attachment.product.categoryName
                        ? ` · ${attachment.product.categoryName}`
                        : ""}
                    </p>
                    <p className="mt-2 text-sm font-semibold text-primary">
                      {formatProductPrice(attachment.product.price)}
                    </p>
                  </div>
                </div>
              </a>
          ...
          if (attachment.type === "order" && attachment.order) {
            const targetPath =
              currentUser?.role === "admin"
                ? `/admin/orders/${attachment.order.id}`
                : `/my-account/orders/${attachment.order.id}`;

            return (
              <a
                key={`order-${attachment.order.id}`}
                href={targetPath}
                target="_blank"
                rel="noreferrer"
                className="block w-64 overflow-hidden rounded-xl border border-slate-200 bg-white transition-all hover:border-primary/50 hover:shadow-md"
              >
                <div className="p-3 flex gap-3">
                  <div className="h-16 w-16 shrink-0 rounded-lg bg-slate-100 overflow-hidden border border-slate-200">
                    {attachment.order.imageUrl ? (
                      <img
                        src={attachment.order.imageUrl}
                        alt={`Order #${attachment.order.id}`}
                        className="h-full w-full object-cover"
                      />
                    ) : (
                  ...
                  <div className="flex flex-col justify-center flex-1 overflow-hidden">
                    <p className="truncate text-sm font-bold text-slate-900">
                      Đơn #{attachment.order.id}
                    </p>
                    <p className="truncate text-xs text-slate-500">
                      {attachment.order.itemsCount} sản phẩm
                    </p>
                    <p className="truncate text-sm font-bold text-primary mt-1">
                      {formatProductPrice(Number(attachment.order.totalAmount))}
                    </p>
                  </div>
                </div>
    ...
    return (
      <div className="mb-3 flex flex-wrap gap-2">
        {imageItems.map(({ file, previewUrl }, index) => (
          <div
            key={`${file.name}-${index}`}
            className="relative w-24 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50"
          >
            <img
              src={previewUrl}
              alt={file.name}
          ...
          >
            ...
            <div className="space-y-1 p-2">
              ...
              <p className="text-[11px] font-semibold text-primary">
                {formatProductPrice(Number(product.price))}
              </p>
            </div>
          </div>
        ))}
        ...
        {selectedOrders.map((order) => (
          <div
            key={`composer-order-${order.id}`}
            className="relative w-40 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 flex flex-col justify-between"
          >
            <div className="h-24 bg-slate-100 flex flex-col items-center justify-center text-slate-400 gap-2 border-b border-slate-200 border-dashed overflow-hidden">
              {order.imageUrl ? (
                <img
                  src={order.imageUrl}
                  alt={`Order #${order.id}`}
                ...
                <>
                  ...
                  <span className="text-xs font-medium">Đơn #{order.id}</span>
                </>
              )}
            </div>
            <div className="space-y-1 p-2">
              ...
              <p className="text-[11px] font-semibold text-primary">
                {formatProductPrice(Number(order.totalAmount))}
              </p>
            </div>
            <button
              type="button"
    ...
    <section className="flex max-h-[calc(100vh-6.5rem)] min-h-[72vh] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white">
        ...
        <div className="text-right text-xs text-slate-500">
          <p>
            {conversation?.status === "HANDLING"
              ? "Shop đang phản hồi"
              : "Đang chờ shop hỗ trợ"}
          </p>
          {conversation?.lastMessageAt ? (
            <p className="mt-1">
              Cập nhật lúc {formatTime(conversation.lastMessageAt)}
            </p>
          ) : null}
        </div>
      </div>
        ...
        >
            ...
            <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
              ...
              <p className="mt-1 text-rose-600">
                {(conversationQuery.error as Error | undefined)?.message ||
                  (messagesQuery.error as Error | undefined)?.message ||
          ...
          {isLoading ? (
            ...
            messages.map((message: ChatMessage) => {
              ...
              const hasAttachmentsContent = Boolean(
      ...
      <Dialog open={isProductPickerOpen} onOpenChange={setIsProductPickerOpen}>
        <DialogContent className="max-h-[86vh] max-w-3xl overflow-y-auto">
          ...
          <div className="grid max-h-[54vh] gap-3 overflow-y-auto p-1 grid-cols-3">
              ...
              availableProducts.map((product) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "relative overflow-hidden rounded-2xl border text-left transition-all flex flex-col group",
                      selected
                  ...
                  >
                    {selected && (
                      <div className="absolute top-2 right-2 z-10 rounded-full bg-primary text-white p-0.5 shadow-sm">
                        <CheckCheck className="size-4" />
                      </div>
                    )}
                    <div
                      className={cn(
                        "aspect-[4/3] w-full bg-slate-100 overflow-hidden relative bg-center bg-cover",
                      )}
                      style={product.images?.[0]?.imageUrl ? { backgroundImage: `url(${product.images[0].imageUrl})` } : undefined}
                      role={product.images?.[0]?.imageUrl ? "img" : undefined}
                      aria-label={product.images?.[0]?.imageUrl ? product.name : undefined}
                    >
                    ...
                    <div className="p-3 flex-1 flex flex-col justify-between w-full">
                      <div>
                        <p
                          className="line-clamp-2 text-xs font-semibold text-slate-900 leading-relaxed"
                          title={product.name}
                        >
                          {product.name}
                        </p>
