...
const OrderDetailPage = () => {
  ...
  const { data, isLoading, isError, error } = useMyOrderById(
    orderId,
    Number.isFinite(orderId),
  );
  const shipmentQuery = useShipmentByOrderId(
    orderId,
    Number.isFinite(orderId) && Boolean(data?.data) && !isLoading && !isError,
  );
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <span className="self-start rounded-md border border-slate-200 px-2 py-1 text-xs text-slate-500 sm:self-auto">
          Đơn #{order.id}
        </span>
      </div>
      ...
      <article className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-4 space-y-3">
          {order.items.map((item) => (
            ...
            >
              ...
              <p className="w-full text-left text-sm font-semibold text-slate-800 sm:w-auto sm:text-right">
        ...
        <div className="mt-4 border-t border-slate-100 pt-4 text-right">
          ...
          <p className="text-sm text-slate-500">
            Phí vận chuyển: {formatCurrency(Number(order.shippingFee))}
          </p>
          <p className="text-lg font-bold text-primary">
            Tổng tiền: {formatCurrency(Number(order.totalAmount))}
          </p>
        </div>
      </article>
    </div>
