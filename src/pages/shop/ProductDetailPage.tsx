                ...
                <section className="space-y-3">
                    <div className="overflow-hidden rounded-lg border border-slate-200 bg-slate-100">
                        {allImages[selectedImage] ? (
                            <img src={allImages[selectedImage]} alt={product.name} className="h-[480px] w-full object-cover" />
                        ) : (
                            <div className="flex h-[480px] w-full items-center justify-center text-slate-500">Không có ảnh</div>
                        )}
                    </div>
                ...
                {isRelatedLoading ? (
                    ...
                    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                        {relatedProducts.map((relatedProduct) => {
                            const relatedImage = relatedProduct.images?.[0]?.imageUrl;
                            const relatedPrice = relatedProduct.variants?.[0]?.price;

                            return (
                                <Link
                                    key={relatedProduct.id}
                                    to={`/${department}/products/${relatedProduct.id}`}
                                    className="overflow-hidden rounded-xl border border-slate-200 bg-white transition hover:-translate-y-0.5 hover:border-primary/35"
                                >
                                    <div className="h-44 bg-slate-100">
                                        {relatedImage ? (
                                            <img src={relatedImage} alt={relatedProduct.name} className="h-full w-full object-cover" />
                                        ) : (
                                            <div className="flex h-full items-center justify-center text-xs text-slate-500">Không có ảnh</div>
                                        )}
                                    </div>
                                    <div className="space-y-1 p-3">
                                        <p className="line-clamp-2 text-sm font-medium text-slate-800">{relatedProduct.name}</p>
                                        <p className="text-sm font-semibold text-primary">{relatedPrice ? formatCurrency(relatedPrice) : "Liên hệ"}</p>
                                    </div>
                                </Link>
                            );
                        })}
